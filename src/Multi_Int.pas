...
v4.83b
-	1.	speedup hex to Multi_Int_XV

v4.83c
-	1.	speedup str to Multi_Int_XV

v4.84b
-	1.	many bug fixes todo with Multi_Int_XV sizing override

v4.84c
-	1.	bug fix in Multi_Int_XV.not
-	2.	tidy-up
...
v4.92
-	1.	speed up multiply_Multi_Int_X2/3/4/5 using multiply_Multi_Int_XV algorithm
-	2.	several bug fixes in Multi_Int_X5 definition
...
const
	Multi_X5_maxi = Multi_X4_maxi + 1;
	Multi_X5_maxi_x2 = Multi_X4_maxi_x2 + 2;
	Multi_X5_size = Multi_X4_size + 1;
	SET_INTERNAL_CALL = TRUE;
type
...
(********************v2********************)
procedure add_Multi_Int_X3(const v1,v2:Multi_Int_X3; var Result:Multi_Int_X3);
var
	TV,CV	:MULTI_INT_2W_S;
	all_zero:boolean;
begin
...
TV:=(MULTI_INT_2W_U(v1.M_Value[5]) + MULTI_INT_2W_U(v2.M_Value[5]) + CV);
if (TV <> 0) then all_zero:= FALSE;
if	(TV > MULTI_INT_1W_U_MAXINT) then
	begin
	Result.Defined_flag:= FALSE;
...
(********************v2********************)
procedure subtract_Multi_Int_X3(const v1,v2:Multi_Int_X3; var Result:Multi_Int_X3);
var
	TV,CV	:MULTI_INT_2W_S;
	all_zero:boolean;
begin
...
TV:=(v1.M_Value[5] - v2.M_Value[5] + CV);
if (TV <> 0) then all_zero:= FALSE;
if	TV < 0 then
	begin
	Result.Defined_flag:= FALSE;
...
then
	...
	then
		begin
		subtract_Multi_Int_X3(v1,v2,Result);
		Neg:= Multi_UBool_TRUE;
		end
	else
		begin
		subtract_Multi_Int_X3(v2,v1,Result);
		Neg:= Multi_UBool_FALSE;
		end;
	end;
...
i:=0; repeat Result.M_Value[i]:= 0; INC(i); until (i > Multi_X3_maxi);
...
repeat
	if (v2.M_Value[j] <> 0) then
		...
		repeat
			...
			INC(i);
		until (i > Multi_X3_maxi);
		i:=0;
        end;
	INC(j);
until (j > Multi_X3_maxi);
...
(********************v2********************)
procedure add_Multi_Int_X4(const v1,v2:Multi_Int_X4; var Result:Multi_Int_X4);
var
...
if	(TV > MULTI_INT_1W_U_MAXINT) then
	begin
	Result.M_Value[5]:= (TV - MULTI_INT_1W_U_MAXINT_1);
	CV:= 1;
	end
else
	begin
...
TV:=(MULTI_INT_2W_U(v1.M_Value[6]) + MULTI_INT_2W_U(v2.M_Value[6]) + CV);
if (TV <> 0) then all_zero:= FALSE;
if	(TV > MULTI_INT_1W_U_MAXINT) then
	begin
	Result.M_Value[6]:= (TV - MULTI_INT_1W_U_MAXINT_1);
	CV:= 1;
	end
else
	begin
...
TV:=(MULTI_INT_2W_U(v1.M_Value[7]) + MULTI_INT_2W_U(v2.M_Value[7]) + CV);
if (TV <> 0) then all_zero:= FALSE;
if	(TV > MULTI_INT_1W_U_MAXINT) then
	begin
	Result.Defined_flag:= FALSE;
...
(********************v2********************)
procedure subtract_Multi_Int_X4(const v1,v2:Multi_Int_X4; var Result:Multi_Int_X4);
var
	TV,CV	:MULTI_INT_2W_S;
	all_zero:boolean;
begin
...
TV:=(v1.M_Value[6] - v2.M_Value[6] + CV);
if (TV <> 0) then all_zero:= FALSE;
if	TV < 0 then
	begin
	Result.M_Value[6]:= (TV + MULTI_INT_1W_U_MAXINT_1);
...
TV:=(v1.M_Value[7] - v2.M_Value[7] + CV);
if (TV <> 0) then all_zero:= FALSE;
if	TV < 0 then
	begin
	Result.Defined_flag:= FALSE;
...
then
	...
	then
		begin
		subtract_Multi_Int_X4(v1,v2,Result);
		Neg:= Multi_UBool_TRUE;
		end
	else
		begin
		subtract_Multi_Int_X4(v2,v1,Result);
		Neg:= Multi_UBool_FALSE;
		end;
	end;
...
i:=0; repeat Result.M_Value[i]:= 0; INC(i); until (i > Multi_X4_maxi);
...
repeat
	if (v2.M_Value[j] <> 0) then
		...
		repeat
			...
			INC(i);
		until (i > Multi_X4_maxi);
		i:=0;
        end;
	INC(j);
until (j > Multi_X4_maxi);
...
if	(TV > MULTI_INT_1W_U_MAXINT) then
	begin
	Result.M_Value[7]:= (TV - MULTI_INT_1W_U_MAXINT_1);
	CV:= 1;
	end
else
	begin
...
TV:=(MULTI_INT_2W_U(v1.M_Value[8]) + MULTI_INT_2W_U(v2.M_Value[8]) + CV);
if (TV <> 0) then all_zero:= FALSE;
if	(TV > MULTI_INT_1W_U_MAXINT) then
	begin
	Result.Defined_flag:= FALSE;
...
TV:=(v1.M_Value[8] - v2.M_Value[8] + CV);
if (TV <> 0) then all_zero:= FALSE;
if	TV < 0 then
	begin
	Result.Defined_flag:= FALSE;
...
(********************v3********************)
procedure Reset_XV_Size(var v1:Multi_Int_XV ;const S:MULTI_INT_2W_U; const INTERNAL_CALL:boolean=FALSE);		{$ifdef inline_functions_level_1} inline; {$endif}
begin
...
var
	b,bi,bs,i,L,p,s,w	:MULTI_INT_2W_S;
	v			:MULTI_INT_2W_U;
	hex_substr		:ansistring;
	Signeg,
	Zeroneg,
...
if All_Zero then
	begin
	Zeroneg:= TRUE;
	Reset_XV_Size(mi, Multi_XV_min_size);
	end;
...
setlength(mi.M_Value, 0);
Reset_XV_Size(MI, Multi_X4_size);
if Multi_Int_ERROR then
	goto OVERFLOW_BRANCH;
...
setlength(mi.M_Value, 0);
Reset_XV_Size(MI, Multi_X3_size);
if Multi_Int_ERROR then
	goto OVERFLOW_BRANCH;
...
setlength(mi.M_Value, 0);
Reset_XV_Size(MI, Multi_X2_size);
if Multi_Int_ERROR then
	goto OVERFLOW_BRANCH;
...
if	(rs > Result.M_Value_Size) then
	begin
	Reset_XV_Size(Result,rs,SET_INTERNAL_CALL);
	if Multi_Int_ERROR then exit;
	end;
...
if INTERNAL_CALL then
	...
	if (rz < rs) then
		begin
		if (rz < ro) then rz:= ro;
		Reset_XV_Size(Result,rz,INTERNAL_CALL);
		if Multi_Int_ERROR then exit;
		end;
	end
else
	begin
	if (rz < Multi_XV_min_size) then rz:= Multi_XV_min_size;
	if (rz >= ro) then
		begin
		Reset_XV_Size(Result, rz);
		if Multi_Int_ERROR then exit;
		end;
	end;
...
(*******************v4B*********************)
procedure INTERNAL_subtract_Multi_Int_XV(const v1,v2:Multi_Int_XV; var Result:Multi_Int_XV; const INTERNAL_CALL:boolean=FALSE);
var
i,cv,
vz1,vz2,
rs,rz,ro,
...
(******************************************)
procedure add_Multi_Int_XV(const v1,v2:Multi_Int_XV; var Result:Multi_Int_XV; const INTERNAL_CALL:boolean=FALSE);
Var	Neg:T_Multi_UBool;
begin
if	(Not v1.Defined_flag)
or	(Not v2.Defined_flag)
...
then
	begin
	INTERNAL_add_Multi_Int_XV(v1,v2,Result,INTERNAL_CALL);
	Neg:= v1.Negative_flag;
	end
else
	if	((v1.Negative_flag = FALSE) and (v2.Negative_flag = TRUE))
	then
		...
		then
			begin
			INTERNAL_subtract_Multi_Int_XV(v2,v1,Result,INTERNAL_CALL);
			Neg:= Multi_UBool_TRUE;
			end
		else
			begin
			INTERNAL_subtract_Multi_Int_XV(v1,v2,Result,INTERNAL_CALL);
			Neg:= Multi_UBool_FALSE;
			end;
		end
	else
...
(******************************************)
procedure subtract_Multi_Int_XV(const v1,v2:Multi_Int_XV; var Result:Multi_Int_XV; const INTERNAL_CALL:boolean=FALSE);
Var	Neg:Multi_UBool_Values;
begin
if	(Not v1.Defined_flag)
or	(Not v2.Defined_flag)
...
while (i < s) do
	begin
	Result.M_Value[i]:= (not v1.M_Value[i]);
	Inc(i);
	end;
...
(*******************v6*********************)
procedure multiply_Multi_Int_XV(const v1,v2:Multi_Int_XV; var Result:Multi_Int_XV; const INTERNAL_CALL:boolean=FALSE); overload;
var
zf		:boolean;
rs,ro,
tv		:MULTI_INT_2W_U;
...
if INTERNAL_CALL then
	...
	if (h < rs) then
		begin
		if (h < ro) then h:= ro;
		Reset_XV_Size(Result,h,INTERNAL_CALL);
		if Multi_Int_ERROR then exit;
		end;
	end
else
...
(******************************************)
procedure multiply_Multi_Int_XV(const v1:Multi_Int_XV; const v2:MULTI_INT_1W_U; var Result:Multi_Int_XV; const INTERNAL_CALL:boolean=FALSE); overload;
var
i,h,rs,ro,
z1	:MULTI_INT_2W_S;
zf	:boolean;
...
repeat
	...
	if	(L > 0) then
		...
		if first_time then
            ...
            multiply_Multi_Int_XV(mi,wf,tm);
			if Multi_Int_ERROR then exit;
			add_Multi_Int_XV(tm,v,mi);
			if Multi_Int_ERROR then exit;
			end;
		if Multi_Int_ERROR then exit;
		if v <> 0 then All_Zero:= FALSE;
...
if	(PT < 0) then R:= 0
	...
	while (PT > 1) do
		...
		if	odd(PT) then
			begin
			multiply_Multi_Int_XV(TV,Y,T,SET_INTERNAL_CALL);
			if Multi_Int_ERROR then exit;
			if	(T.Negative_flag = Multi_UBool_UNDEF) then
				if	(TV.Negative_flag = Y.Negative_flag)
				then T.Negative_flag:= Multi_UBool_FALSE
		...
		multiply_Multi_Int_XV(TV,TV,T,SET_INTERNAL_CALL);
		if Multi_Int_ERROR then exit;
		T.Negative_flag:= Multi_UBool_FALSE;
	...
	multiply_Multi_Int_XV(TV,Y,R,SET_INTERNAL_CALL);
	if Multi_Int_ERROR then exit;
	if	(R.Negative_flag = Multi_UBool_UNDEF) then
		if	(TV.Negative_flag = Y.Negative_flag)
		then R.Negative_flag:= Multi_UBool_FALSE
...
(********************v0********************)
procedure intdivide_taylor_warruth_XV(const P_dividend,P_dividor:Multi_Int_XV;out P_quotient,P_remainder:Multi_Int_XV; const INTERNAL_CALL:boolean=FALSE);
label	AGAIN, FINISH, RESIZE;
...
if	(P_dividor = 0) then
	...
	orig_size:= P_dividend.M_Value_Size;
	Reset_XV_Size(P_quotient, P_dividend.M_Value_Size, SET_INTERNAL_CALL);
	Reset_XV_Size(P_remainder, P_dividend.M_Value_Size, SET_INTERNAL_CALL);
	...
	Reset_XV_Size(dividor, div_size, SET_INTERNAL_CALL);
	Reset_XV_Size(dividend, div_size, SET_INTERNAL_CALL);
	Reset_XV_Size(quotient, div_size, SET_INTERNAL_CALL);
	Reset_XV_Size(next_dividend, div_size, SET_INTERNAL_CALL);
	Reset_XV_Size(temp_dividend, div_size, SET_INTERNAL_CALL);
	...
	do
		...
		if	(word_division > 0) then
            ...
            multiply_Multi_Int_XV(dividor, quotient, temp_dividend, SET_INTERNAL_CALL);
			subtract_Multi_Int_XV(dividend,temp_dividend,next_dividend, SET_INTERNAL_CALL);
			if Multi_Int_ERROR then
				begin
				P_quotient.Defined_flag:= FALSE;
				P_quotient.Overflow_flag:= TRUE;
...
RESIZE:
	...
	if INTERNAL_CALL then
		...
		if (hi < div_size) then
			begin
			if (hi < orig_size) then hi:= orig_size;
			Reset_XV_Size(P_quotient,hi,INTERNAL_CALL);
			Reset_XV_Size(P_remainder,hi,INTERNAL_CALL);
			if Multi_Int_ERROR then exit;
			end;
		end
	else
...
repeat
	begin
	// CC:= ((C + (v1 div C)) div 2);
    intdivide_taylor_warruth_XV(v1, C, Q, R, SET_INTERNAL_CALL);
	if (Multi_Int_ERROR) then exit;

	add_Multi_Int_XV(C, Q, CC, SET_INTERNAL_CALL);
	if (Multi_Int_ERROR) then exit;

    ShiftDown_MultiBits_Multi_Int_XV(CC, 1);

	subtract_Multi_Int_XV(C, CC, TC, SET_INTERNAL_CALL);
	if (Multi_Int_ERROR) then exit;
...
multiply_Multi_Int_XV(LPC, LPC, TC, SET_INTERNAL_CALL);
if (Multi_Int_ERROR) then exit;

subtract_Multi_Int_XV(v1, TC, VREM, SET_INTERNAL_CALL);
if (Multi_Int_ERROR) then exit;
...
if Multi_Int_Initialisation_done then
	...
	Reset_XV_Size(Multi_Int_XV_MAXINT, Multi_XV_limit);
	i:=0;
	while (i < Multi_Int_XV_MAXINT.M_Value_Size ) do
		begin
		Multi_Int_XV_MAXINT.M_Value[i]:= MULTI_INT_1W_U_MAXINT;
