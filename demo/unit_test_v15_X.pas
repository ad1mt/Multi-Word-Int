...
{$IFDEF CPU64}
	{$define 64bit}
	XV_SIZE:= 17;	// the 64bit size must be 18 otherwise large constants later must be changed
	Multi_Int_Initialisation(XV_SIZE);
	Multi_Int_Set_XV_Limit(18);
{$ELSE}
  	{$define 32bit}
	XV_SIZE:= 36;	// the 64bit size must be 36 otherwise large constants later must be changed
	Multi_Int_Initialisation(XV_SIZE);
{$ENDIF}
